...
const readMessage = (provider, buf, emitSynced) => {
	...
	console.log("LENGTH", bufLength - 1)
	...
	switch (messageType) {
		...
		case MESSAGE.QUERY_AWARENESS:
			...
			encoding.writeVarUint(encoder, MESSAGE.QUERY_AWARENESS)
			break;
		default:
	...
	if (provider.shouldConnect && provider.ws === null) {
		...
		websocket.onopen = () => {
			...
			if (provider.awareness.getLocalState() !== null) {
				...
				// websocket.send(encoding.toUint8Array(encoderAwarenessState).slice(1))
			}
		}
		provider.emit('status', [{
			status: 'connecting'
